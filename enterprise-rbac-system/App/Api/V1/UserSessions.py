...
def get_user_sessions(
    ...
    try:
        ...
        for session in sessions:
            ...
            # 添加用户名和昵称
            if session.user:
                session_dict["username"] = session.user.username
                session_dict["nickname"] = session.user.nickname
...
def count_expired_sessions(
    ...
    try:
        current_user, tenant_id = current_user_with_tenant
        session_service = UserSessionService(db)
        count_info = session_service.count_expired_sessions(tenant_id=tenant_id)
...
def clean_expired_sessions(
    ...
    try:
        current_user, tenant_id = current_user_with_tenant
        session_service = UserSessionService(db)
        result = session_service.clean_expired_sessions(tenant_id=tenant_id)
...
def get_user_session(
    ...
    try:
        ...
        # 添加用户名和昵称
        if session.user:
            session_info["username"] = session.user.username
            session_info["nickname"] = session.user.nickname
