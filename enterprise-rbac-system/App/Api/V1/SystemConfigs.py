...
@router.get("/groups", response_model=AuthResponse, summary="获取配置分组列表", dependencies=[Depends(permission_dependency("config:view"))])
...
@router.get("/grouped", response_model=AuthResponse, summary="获取分组配置", dependencies=[Depends(permission_dependency("config:view"))])
...
@router.get("/active", response_model=AuthResponse, summary="获取活跃配置", dependencies=[Depends(permission_dependency("config:view"))])
...
@router.get("/key/{key}", response_model=AuthResponse, summary="根据键获取配置值", dependencies=[Depends(permission_dependency("config:view"))])
...
def update_config(
    ...
    try:
        current_user, tenant_id = current_user_with_tenant
        config_service = SystemConfigService(db)
        config = config_service.update_config(config_id, config_data, tenant_id=tenant_id, user_id=current_user.id)
...
@router.post("/batch-update", summary="批量更新配置", dependencies=[Depends(permission_dependency("config:update"))])
async def batch_update_configs(
    ...
    """批量更新配置接口"""
    try:
        configs_data = await request.json()
        
        current_user, tenant_id = current_user_with_tenant
        config_service = SystemConfigService(db)
        
        str_configs_data = {k: str(v) if v is not None else "" for k, v in configs_data.items()}
        updated_configs = config_service.batch_update_configs(str_configs_data, tenant_id=tenant_id, user_id=current_user.id)
