...
class SystemDictService:
    ...
    def update_dict(self, dict_id: int, dict_update: SystemDictUpdate, tenant_id: Optional[int] = None, user_id: Optional[int] = None) -> SystemDictModel:
        """更新字典"""
        dict_obj = self.get_dict_by_id(dict_id, tenant_id)
        ...
        # 手动更新字段
        if dict_update.name is not None:
            dict_obj.name = dict_update.name
        if dict_update.code is not None:
            dict_obj.code = dict_update.code
        if dict_update.description is not None:
            dict_obj.description = dict_update.description
        if dict_update.sort is not None:
            dict_obj.sort = dict_update.sort
        if dict_update.status is not None:
            dict_obj.status = dict_update.status

        result = self.system_dict_repository.update(dict_obj, user_id)
    ...
    def update_dict_item(self, item_id: int, item_update: SystemDictItemUpdate, tenant_id: Optional[int] = None, user_id: Optional[int] = None) -> SystemDictItemModel:
        """更新字典项"""
