...
class SystemConfigService:
    ...
    def update_config(self, config_id: int, config_update: SystemConfigUpdate, tenant_id: Optional[int] = None, user_id: Optional[int] = None) -> SystemConfigModel:
        """更新配置"""
        ...
        if config_update.sort is not None:
            config.sort = config_update.sort
        if config_update.status is not None:
            config.status = config_update.status

        result = self.system_config_repository.update(config, user_id)
    ...
    def batch_update_configs(self, configs_data: Dict[str, str], tenant_id: int, user_id: Optional[int] = None) -> List[SystemConfigModel]:
        """批量更新配置，不存在则创建"""
        updated_configs = []
        for key, value in configs_data.items():
            try:
                ...
                updated_config = self.system_config_repository.update(config, user_id)
                updated_configs.append(updated_config)
            except NotFoundException:
                import logging
                logger = logging.getLogger(__name__)
                logger.info(f"配置 {key} 不存在，创建新配置")
                from App.Schemas.SystemConfig import SystemConfigCreate
                ...
                new_config = self.create_config(config_create, tenant_id)
