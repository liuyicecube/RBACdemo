...
class SystemDictRepository(BaseRepository[SystemDictModel]):
    ...
    def get_by_code(self, code: str, tenant_id: int) -> Optional[SystemDictModel]:
        ...
        return self.db.query(SystemDictModel).filter(
            ...
            or_(SystemDictModel.tenant_id == tenant_id, SystemDictModel.tenant_id.is_(None)),
...
class SystemDictItemRepository(BaseRepository[SystemDictItemModel]):
    ...
    def get_by_dict_id(self, dict_id: int, tenant_id: Optional[int] = None, skip: int = 0, limit: int = 100) -> List[SystemDictItemModel]:
        """根据字典ID获取字典项"""
        ...
        if tenant_id is not None:
            query = query.filter(or_(
                ...
                SystemDictItemModel.tenant_id.is_(None)
            ))
        return query.order_by(SystemDictItemModel.sort, SystemDictItemModel.create_time.desc()).offset(skip).limit(limit).all()

    def get_active_items_by_dict_id(self, dict_id: int, tenant_id: Optional[int] = None) -> List[SystemDictItemModel]:
        """根据字典ID获取活跃字典项"""
        query = self.db.query(SystemDictItemModel).filter(
            SystemDictItemModel.dict_id == dict_id,
            SystemDictItemModel.is_deleted == 0,
        ...
        return query.order_by(SystemDictItemModel.sort, SystemDictItemModel.create_time.desc()).all()
    ...
    def paginate(
        ...
        """分页查询字典项"""
        query = self.db.query(SystemDictItemModel).filter(
            or_(SystemDictItemModel.tenant_id == tenant_id, SystemDictItemModel.tenant_id.is_(None)),
            SystemDictItemModel.is_deleted == 0
        )
