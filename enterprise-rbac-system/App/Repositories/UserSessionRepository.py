...
class UserSessionRepository(BaseRepository[UserSessionModel]):
    ...
    def get_by_id(self, id: int, tenant_id: int) -> Optional[UserSessionModel]:
        """根据ID获取记录（重写，预加载用户信息）"""
        return self.db.query(UserSessionModel).options(
            joinedload(UserSessionModel.user)
        ).filter(
            UserSessionModel.id == id,
    ...
    def count_expired_sessions(self, tenant_id: int) -> dict:
        ...
        # 统计已过期的会话
        expired_count = self.db.query(UserSessionModel).filter(
            UserSessionModel.tenant_id == tenant_id,
            UserSessionModel.is_deleted == 0,
            UserSessionModel.expire_time <= datetime.now()
        ).count()
        ...
        # 统计已失效的会话（状态为0）
        expired_status_count = self.db.query(UserSessionModel).filter(
            UserSessionModel.tenant_id == tenant_id,
            UserSessionModel.is_deleted == 0,
            UserSessionModel.status == 0
        ).count()
        ...
        return {
            ...
            "expired_status_count": expired_status_count,
            "total_to_clean": expired_count + expired_status_count
        }
    ...
    def clean_expired_sessions(self, tenant_id: int) -> dict:
        ...
        expired_count = self.db.query(UserSessionModel).filter(
            UserSessionModel.tenant_id == tenant_id,
            UserSessionModel.is_deleted == 0,
            UserSessionModel.expire_time <= now
        ).count()
        ...
        return {
            "expired_count": expired_count,
            "expired_status_count": expired_status_count,
            "total_cleaned": expired_count + expired_status_count
        }
    ...
    def paginate(
        ...
        """分页查询用户会话"""
        query = self.db.query(UserSessionModel).options(
            joinedload(UserSessionModel.user)
        ).filter(
            UserSessionModel.tenant_id == tenant_id,
            UserSessionModel.is_deleted == 0
